...
namespace iWorks.Common.Dapper
{
...
this IDbConnection cnn, string sql, dynamic param = null, IDbTransaction transaction = null, int? commandTimeout = null, CommandType? commandType = null
        ...
        public static Action<IDbCommand, TObject> GetExecutNonQueryActionImpl<TObject>(this IDbConnection connection, string sql, TObject paramObject)
        {
            var command = new CommandDefinition(sql, paramObject, null, null, null, CommandFlags.Buffered);
            object param = command.Parameters;
            IEnumerable multiExec = param as IEnumerable;
            Identity identity;
            var cmd = command.SetupCommand(connection, null);
            identity = new Identity(command.CommandText, cmd.CommandType, connection, null, paramObject.GetType(), null);
            var literals = GetLiteralTokens(identity.sql);

            return IntializeCommandGenerator<TObject>(identity, false, true, literals);
        }
...
#pragma warning restore 0618
        ...
        internal static Action<IDbCommand, TType> IntializeCommandGenerator<TType>(Identity identity, bool checkForDuplicates, bool removeUnused, IList<LiteralToken> literals)
        {
            Type type = identity.parametersType;
            ...
            //Set command text here only
            il.Emit(OpCodes.Nop);
            il.Emit(OpCodes.Ldarg_0);
            il.Emit(OpCodes.Ldstr, identity.sql);
            il.EmitCall(OpCodes.Callvirt, typeof(IDbCommand).GetProperty("CommandText").GetSetMethod(), null);
            il.Emit(OpCodes.Nop);
            ...
            return (Action<IDbCommand, TType>)dm.CreateDelegate(typeof(Action<IDbCommand, TType>));
        }
...
Type type, IDataReader reader, int startBound = 0, int length = -1, bool returnNullIfFirstMissing = false
        ...
        public static Func<IDataReader, object> GetTypeDeserializer<TType>(IDataReader datareader)
        {
            return GetTypeDeserializer(typeof(TType), datareader, 0, -1, false);
        }
